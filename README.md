...
```bash
# 根据IRTI风险代码推荐参考标的（仅供研究）
python agent.py recommend F2-E2-C2-T2

# 查看所有风险等级对应标的
python agent.py recommend --list
```

**风险代码格式**: `F{1-4}-E{1-4}-C{1-4}-T{1-4}`
- F: 财务承载力（1=脆弱, 2=稳健, 3=强韧, 4=极高）
- E: 情绪波动性（1=敏感, 2=中庸, 3=冷静, 4=冷漠）
- C: 认知复杂度（1=基础, 2=进阶, 3=专业, 4=构建）
- T: 流动性周期（1=超短钱, 2=战术钱, 3=战略钱, 4=永续钱）

⚠️ **重要提示**：以上推荐仅供学术研究参考，不构成任何投资建议。股市有风险，投资需谨慎。

### 4.3 Claude Code 启动方式

```bash
# 查看Claude Code集成方式
python agent.py claude

# 在 Claude Code 中直接使用
# $ claude "分析贵州茅台 600519 的基本面"
# $ claude "我的IRTI测试结果是F2-E2-C2-T2，推荐什么股票"
```

**四种集成方式**：
1. **Claude Code 直接调用**：在对话中输入分析指令
2. **CLAUDE.md 注册**：在项目中添加可用命令说明
3. **MCP 工具注册**：配置为 Claude Code MCP server
4. **CLI 直接使用**：本地 Python 直接运行
